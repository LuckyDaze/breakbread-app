...
from app.notifications import toast_success, toast_info, toast_warn
...
# Small utility to normalize user input symbols
...
def show_login():
    ...
    with col1:
        ...
        password = st.text_input("Password", type="password", value="demo123")  # demo only

        if st.button("Login", type="primary"):
            result = fake_login(app_id)
            if result["status"] == "2FA_REQUIRED":
                st.session_state.login_2fa_code = result.get("code_sent")  # placeholder in demo
                st.info("2FA code sent to your device (demo). Enter any 6-digit code on the right.")
            else:
                st.error(result.get("message", "Login failed"))

    with col2:
        if st.session_state.login_2fa_code:
            st.subheader("Enter 2FA Code")
            code = st.text_input("6-digit code", placeholder="123456")
            if st.button("Verify Code"):
                result = fake_login(st.session_state.login_2fa_code, code)
...
def show_dashboard(user):
    ...
    with col3:
        allocation = allocation_breakdown(user["user_id"])
        crypto_pct = allocation.get("crypto", 0) / total_value * 100 if total_value > 0 else 0
            ...
            user_activities.append(
                {
                    ...
                    "Description": f"{order['units']} units @ {format_money(order['fill_price'])}",
...
def show_markets(user):
    ...
    with col1:
        raw_symbol = st.text_input("Symbol", value="AAPL")
...
def show_portfolio(user):
    ...
    with col3:
        allocation = allocation_breakdown(user["user_id"])
        st.metric("Positions", len(user["portfolio"]))
    ...
    with col1:
        trade_raw_symbol = st.text_input("Symbol", value="AAPL", key="trade_symbol")
        symbol = _clean_symbol(trade_raw_symbol)
    with col2:
    ...
    with col3:
        if side == "buy":
            ...
            units = st.number_input("Units", min_value=0.01, value=1.0, step=0.1)
            cash_amount = None
    with col4:
        st.write("")  # Spacer
        if st.button("Place Order", type="primary"):
            ...
            if success:
                toast_success(f"Order filled: {side} {order['units']} {symbol}")
                st.rerun()
            else:
                st.error(message)
            ...
            if data:
                ...
                with col3:
                    if st.button("View", key=f"view_{sym}"):
                        st.session_state.view_symbol = sym  # handy if you wire it to Markets
                with col4:
...
def show_settings(user):
    ...
    with col2:
        ...
        alert_symbol = _clean_symbol(st.text_input("Symbol for Alert", value="BTC-USD"))
        threshold = st.number_input("Alert Threshold ($)", min_value=0.01, value=50000.0)

        if st.button("Set Alert"):
            if alert_symbol:
                user["settings"]["price_alerts"][alert_symbol] = threshold
                toast_success(f"Alert set for {alert_symbol} at {format_money(threshold)}")
            else:
