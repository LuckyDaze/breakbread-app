    ...
    with tab2:
        ...
        if st.session_state.transactions:
            for tx in reversed(st.session_state.transactions[-10:]):
                st.write(f"**{tx['type']}** - {tx['amount']} - {tx['date']}")
                if tx.get('note'):
                    st.write(f"*{tx['note']}*")
                st.divider()
        else:
...
def show_main_app():
    ...
    if selected_nav == "Dashboard":
        show_dashboard(user)
    elif selected_nav == "Banking":
        show_banking(user)
    elif selected_nav == "Markets":
        show_markets(user)
